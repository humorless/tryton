...
| `purchase` 模組（Day 2 前）/ `sale` 模組（Day 4 前）已啟用 | ✅ 已完成（`sale` 於 2026-07-09 啟用，見 [`admin-guide.md`](admin-guide.md) 「Day 4 前置」） |
| `aaa` 加入 **Sales／Sales Administrator** 群組（Day 4 前）| ✅ 已完成（2026-07-09，psql 複查，見 [`admin-guide.md`](admin-guide.md) 「Day 4 前置（續）」） |
| `aaa` 加入 **Accounting** 群組（Day 5 前）| ✅ 已完成（2026-07-13，psql 複查，見 [`admin-guide.md`](admin-guide.md) 「Day 5 前置」） |
...
- 狀態：✅ 完成（2026-07-09，Invoice／Payment 留給 Day 5）
- 前置：admin 已啟用 `sale` 模組、`aaa` 已加入 Sales／Sales Administrator 群組（見 [`admin-guide.md`](admin-guide.md) 「Day 4 前置」）
- 環境現況：`test customer`（party id=3）Day 1 就已建好，不用重新建立客戶；書（bk001）在 `product_list_price` 已有 `list_price=680`，可直接沿用

### Step 1：把書設成「可銷售」

- 狀態：✅ 完成（2026-07-09）
- 選單路徑：Products → 打開 `bk001`（從試算表到資料平台）→ **Customers** 分頁
- **實測發現：`Salable` 跟 `Sale UoM` 是綁在一起強制同時填的**——只勾 Salable 不選 Sale UoM 存不了檔，反之亦然；這跟 Day 2 `Purchasable`／`Purchase UoM` 是同一組設計（一個 Product 能不能賣/採購，跟「用什麼單位賣/採購」是綁定在一起決定的，不能只設一半）
- 填法：勾選 **Salable**、Sale UoM 選 **Unit**
- 📸 `day4-user-01-product-salable-setup.png`

**執行結果（psql 複查，2026-07-09）**：`product_template` id=1，`salable=true`，`sale_uom` 指向 Unit——跟預期一致，`sale.line` 的 Product 欄位有 `domain=[('salable','=',True), ...]`（查 `trytond/modules/sale/sale.py`），沒設這個欄位的話銷售單根本選不到這個商品。

### Step 2：建立銷售單（Sale Order）

- 選單路徑：**Sales ‣ Sales** → 工具列 **+** 新增

**要填的欄位**

| 欄位 | 填什麼 | 說明 |
|---|---|---|
| Party | `test customer` | Day 1 已建好，直接選 |
| Payment Term | **Net 30 days** | 查了 `account.configuration` 沒有預設值（`default_customer_payment_term` 沒設），所以這欄不會自動帶出來，要手動選；`payment_term` 欄位本身在 Draft／Quotation 不是必填，但一旦要 Confirm（離開 Draft/Quotation/Cancelled 狀態）就會變必填，晚選不如現在就選 |
| Warehouse | 保持預設 | `default_warehouse()` 會自動抓公司唯一的 Warehouse，不用手動選 |
| Currency | 保持預設 | 自動帶公司幣別 |
| Lines：Product | `bk001`（從試算表到資料平台） | Step 1 剛設成 Salable，才選得到 |
| Lines：Quantity | **2** | 目前 Storage Zone 淨庫存 6、Output Zone 淨庫存 3，選 2 本足夠示範又留有餘裕 |
| Lines：Unit Price | 保持自動帶出的 680 | 來自 `product_list_price`，Day 1 建 Product 時設定的售價 |

存好草稿後截圖給我，我再幫你查 psql 複查。

- 📸 `day4-user-02-sale-draft-form.png`（Party=test customer、Payment Term=Net 30 days、Warehouse=[WH] Warehouse、Line：bk001／2u／680.00，Amount=1,360.00）

**執行結果（psql 複查，2026-07-09）**

| 表 | 欄位 | 值 |
|---|---|---|
| `sale_sale` | id / number / party / payment_term / warehouse / state | 1 ／（空）／ 3（test customer）／ 4（Net 30 days）／ 5（Warehouse）／ `draft` |
| `sale_line` | id / sale / product / quantity / unit_price | 1 ／ 1 ／ 2（bk001 變體）／ 2 ／ 680 |

- **`number` 欄位還是空的**——跟 Day 2 的 Purchase Order、Day 3 舊版的 Internal Shipment 是同一套規則：Draft 階段不跟 Sequence 要號，等真正脫離 Draft 才給號，避免草稿被刪掉時序號留下缺口
- 下一步：點 **Quote** 進入 Quotation 狀態（對應「把報價單開給客戶」），再 **Confirm** 進入 Confirmed（客戶答應了、正式承諾要賣），最後 **Process** 才會真正產生 Customer Shipment／Invoice——這三段對應到 Day 2 Purchase Order 的 Quote／Confirm／Process，是同一套狀態機設計

**概念筆記：Sequence Number 將來要怎麼用？是拿來除錯還是查核？**

- 這是兩套完全不同的機制，不要混為一談
- **Number 是給「人」看的**：`sale.sale` 的 `_rec_name = 'number'`，代表這張單在下拉選單、搜尋結果、關聯欄位裡預設顯示的就是這個編號，不是資料庫的 `id`；印出來的報價單/確認單上也是印這個號碼——客戶說「我的訂單 SAL1」，靠的就是這個。查了 `order_number()` classmethod，List View 的預設排序也是照這個號碼排
- **真正的除錯／追溯鏈，靠的是另一個欄位：`origin`**——查原始碼發現，出貨時 `move.origin = self`（這個 sale.line）、開發票時 `invoice_line.origin = self`。這是一個 Reference 欄位（存 `model,id`，例如 `sale.line,1`），程式要查「這筆 Stock Move／Invoice Line 是哪張 Sale 產生的」，直接查這個欄位 join，不是拿 Number 字串去比對
- **這也解釋了為什麼 Sale 的號碼不是 Strict Sequence**：查了 `sale/configuration.py`，`sale_sequence` 欄位型別是普通的 `ir.sequence`，不是 Invoice 用的 `ir.sequence.strict`（絕對禁止跳號，資料庫層級鎖死）。差別在於：Invoice 這種法定會計憑證，跳號在稽核上不可接受；但 Sale Order 不是法定憑證，只是業務流程的起點，就算極端情況下出現跳號，也不影響真正需要靠 `origin` 追溯的稽核鏈
- **結論**：Sequence Number 主要拿來做人類辨識、對外溝通、UI 排序/搜尋，某種程度上也算除錯輔助（人跟人對話時報號碼），但不是系統拿來做資料完整性稽核的機制；真正的稽核／回溯，走的是 `origin` 這個關聯欄位

**實測確認（2026-07-09）：點 QUOTE 之後**

- 點 Quote 前補截了一張完整畫面（含下方 Invoice State／Shipment State／State／footer 按鈕）：
  - 📸 `day4-user-03-sale-draft-footer.png`（State=Draft，Invoice State=None，Shipment State=None，footer 按鈕為 CANCEL／MODIFY HEADER／QUOTE）
- 點 Quote 之後：
  - 📸 `day4-user-04-sale-quotation-state.png`（State=Quotation，Quotation Date 自動填today＝07/09/2026，footer 按鈕變成 CANCEL／DRAFT／CONFIRM）

**執行結果（psql 複查，2026-07-09）**：`sale_sale` id=1，`number=1`（剛剛還是空的，`quote()` 呼叫 `set_number()` 之後才給號，跟前面概念筆記推演的一致）、`state=quotation`、`quotation_date=2026-07-09`。

- footer 按鈕從 QUOTE 變成 DRAFT／CONFIRM，對應 `_transitions`：`quotation` 狀態可以 `confirm`（往前，客戶答應了）也可以 `draft`（往回，退回修改）或 `cancel`；這跟 Day 2 Purchase Order 的 Quote 之後狀態機是同一套設計
- 下一步：點 **CONFIRM**，讓 Sale 從 Quotation 進入 Confirmed（正式承諾要賣，之後才能 Process 產生 Invoice／Customer Shipment）

**實測確認（2026-07-09）：點 CONFIRM 之後，直接跳到 Processing，Invoice／Shipment 都已經自動產生**

- 📸 `day4-user-05-sale-confirm-auto-processing.png`（State 直接是 **Processing**，不是 Confirmed；Invoice State=Pending，Shipment State=Waiting，下方 SHIPMENTS 徽章=1、INVOICES 徽章=1，footer 只剩 PROCESS 按鈕）
- **這不是新機制，是 Day 2 Purchase Order 已經查證過的同一套自動排程行為**（見上方 Day 2 Step 2「Process 按鈕跟 `ir.queue`／`trytond-worker` 的關係」）：`confirm()` 一結束就會呼叫 `cls.__queue__.process(sub_sales)` 排一個 `process()` 任務；因為這個環境 `trytond.conf` 沒設定 `[queue] worker`，任務會在同一個 request commit 後**立刻同步執行**，所以肉眼看起來像是「按 Confirm 就自動 Process 了」——這次沒有卡關，代表 Day 2 修好的 Account Category 設定（書籍 Category 的 Accounting／Account Revenue）這次也對銷售端生效，不用重修一次

**執行結果（psql 複查，2026-07-09）**

| 表 | 欄位 | 值 |
|---|---|---|
| `sale_sale` | state / invoice_method / invoice_state / shipment_method / shipment_state | `processing` ／ `order` ／ `pending` ／ `order` ／ `waiting` |
| `account_invoice` | id / number / state / type / party | 2 ／（空）／ `draft` ／ `out`（銷項）／ 3（test customer） |
| `stock_shipment_out` | id / number / state / warehouse / customer | 1 ／ 1 ／ `waiting` ／ 5（Warehouse）／ 3（test customer） |
| `stock_move` | id / product / quantity / from→to / state / shipment / origin | 6 ／ bk001 ／ 2 ／ Output Zone→Customer ／ `draft` ／ `stock.shipment.out,1` ／ `sale.line,1` |
| `stock_move`（同一批，另一筆） | id / product / quantity / from→to / state / shipment / origin | 7 ／ bk001 ／ 2 ／ Storage Zone→Output Zone ／ `draft` ／ `stock.shipment.out,1` ／ `stock.move,6` |

**概念筆記：Customer Shipment 自動產生了「兩段式」Stock Move，不是一步到位**

- 這是這次新發現、Day 2 Purchase 沒遇到的細節：`_process_fulfillment` 幫這張 Sale 產生 Customer Shipment 時，實際建立了**兩筆串接的 Move**，不是一筆「Storage Zone 直接搬到 Customer」：
  1. **Storage Zone → Output Zone**（move id=7，`origin=stock.move,6`）：對應「先把書從架上揀出來，集中到出貨暫存區」——這就是 Day 3 Step 2 手動示範過的「揀貨準備出貨」動作，這次是系統自動幫你做的，不用再手動建一張 Internal Shipment
  2. **Output Zone → Customer**（move id=6，`origin=sale.line,1`）：對應「真正把書交給客戶、離開店裡」，這才是這張 Sale Line 真正掛勾的那筆 Move
- **跟 Day 3 手動示範的呼應**：Day 3 Step 2 手動做的 Storage→Output 搬運，其實就是在**模擬** Customer Shipment 正常流程裡系統會自動產生的第一段 Move；差別在於這次是「銷售驅動」自動觸發，Day 3 是「為了學 Internal Shipment 這個 model」刻意手動做一次同樣性質的動作
- 兩筆 Move 目前都還是 `draft`，Shipment 整體也還是 `waiting`——下一步要點 **Assign**，系統才會真的去檢查 Storage Zone 有沒有 2 本書可以撥出來

### Step 3：Customer Shipment 的完整狀態機（比 Internal Shipment 多兩階）

- 開啟方式：Sale 表單下方 **SHIPMENTS** 徽章 → 點進清單裡唯一一筆 Shipment
- 📸 `day4-user-06-shipment-waiting-before-assign.png`（State=Waiting，Inventory Moves 分頁：`[STO] Storage Zone`／bk001／2u／`Draft`，footer：CANCEL／DRAFT／WAIT／ASSIGN）
- 📸 `day4-user-07-shipment-assigned-state.png`（點 Assign 之後，State=Assigned，同一筆 Inventory Move 變成 `Assigned`，footer 變成 CANCEL／WAIT／PICK）

**執行結果（psql 複查，2026-07-09）**

| 表 | 欄位 | 值 |
|---|---|---|
| `stock_shipment_out` | id / state | 1 ／ `assigned` |
| `stock_move`（Inventory Move，Storage→Output） | id / state | 7 ／ `assigned` |
| `stock_move`（Outgoing Move，Output→Customer） | id / state | 6 ／ **仍是 `draft`，還沒變** |

**概念筆記：`stock.shipment.out` 的狀態機比 Day 3 的 Internal Shipment 多兩階，因為它是真的「兩段搬運」**

- 查了 `trytond/modules/stock/shipment.py` 的 `ShipmentOut.state`：完整狀態是 **Draft → Waiting → Assigned → Picked → Packed → Shipped → Done**，比 Day 3 學過的 Internal Shipment（Draft/Waiting/Assigned/[可跳過 Packed／Shipped]/Done）多了 **Picked** 這一階，`_transitions` 也更複雜（可以在 Assigned／Waiting／Packed 之間來回調整）
- **原因跟這張單真的產生「兩筆」Move 有關**——Internal Shipment 只包一筆 Move（單一次搬運），Customer Shipment 因為書要先「上架庫存→出貨暫存區」再「出貨暫存區→客戶」，天生就是兩段式，狀態機自然要多切一階讓「揀貨」跟「打包」分開驗證
- 查了各按鈕實際做的事：
  - **Assign**：只處理 `inventory_moves`（Storage→Output 這一段），呼叫 `Move.assign()` 檢查／保留 Storage Zone 的庫存——這次成功了，因為 Storage Zone 淨庫存還有 6，夠撥 2 本
  - **Pick**：呼叫 `Move.do(inventory_moves)`，真正執行 Storage→Output 這筆 Move（Storage Zone 的書實體「搬到」Output Zone），同時把 `outgoing_moves`（Output→Customer）從無到有變成 `draft`，準備進入下一階段——這對應「揀貨動作完成，書已經實際放到出貨暫存區了」
  - **Pack**：呼叫 `Move.assign(outgoing_moves)`，這時候才檢查／保留 Output Zone 的庫存（因為 Pick 才剛把書放進 Output Zone，這一步驗證的就是「揀貨動作有沒有確實完成」）——對應「打包完成，準備交運」
- **一句話**：Assign 管『倉庫拿不拿得出貨』，Pick 管『拿出來放到出貨區這個動作本身』，Pack 管『打包前再次確認出貨區真的有貨』——三個按鈕分別對應「查庫存」「揀貨」「打包」三個獨立的實體動作，不是同一件事拆成三次點擊
- 下一步：點 **PICK**，觀察 Storage Zone／Output Zone 的實際庫存數字變化

**實測確認（2026-07-09）：點 PICK 之後，庫存數字真的變動了**

- 📸 `day4-user-08-shipment-picked-state.png`（State=Picked，Inventory Move 變成 `Done`，footer 只剩 CANCEL／PACK）

**執行結果（psql 複查，2026-07-09）**

| 表 | 欄位 | 值 |
|---|---|---|
| `stock_shipment_out` | id / state | 1 ／ `picked` |
| `stock_move`（Inventory Move） | id / state / effective_date | 7 ／ `done` ／ 2026-07-09 |
| `stock_move`（Outgoing Move） | id / state | 6 ／ 仍是 `draft`（Pick 只執行第一段，不動第二段） |

**淨庫存變化（psql 複查，2026-07-09）**

| Location | Pick 前 | Pick 後 |
|---|---|---|
| Storage Zone | 6 | **4** |
| Output Zone | 3 | **5** |
| Customer | 0 | 0（還沒變，要等最後 Ship／Done） |

跟前面概念筆記推演的完全一致：Pick 執行了 Storage→Output 這筆 Move，書實體「搬到」了出貨暫存區，Output→Customer 那筆還在 Draft 等下一步 Pack 驗證。

下一步：點 **PACK**，這時候系統才會真的去檢查 Output Zone 有沒有 2 本書可以交運。

**實測確認（2026-07-09）：點 PACK 之後**

- 📸 `day4-user-09-shipment-packed-state.png`（State=Packed，footer 變成 CANCEL／PICK／**SHIP**／**COMPLETE**——同時出現兩個看起來像「下一步」的按鈕）

**執行結果（psql 複查，2026-07-09）**

| 表 | 欄位 | 值 |
|---|---|---|
| `stock_shipment_out` | id / state | 1 ／ `packed` |
| `stock_move`（Outgoing Move） | id / state | 6 ／ `assigned`（Pack 檢查／保留了 Output Zone 的庫存，跟概念筆記推演一致） |

**概念筆記：Packed 狀態下同時出現 SHIP 跟 COMPLETE，是不是重複了？**

- 查了 `_buttons`：`ship` 的顯示條件是 `state == 'packed'`；`do`（COMPLETE）的顯示條件是 `state in ['packed', 'shipped']`——**兩個按鈕本來就會同時出現在 Packed 這一階，不是重複，是刻意讓操作者選**
- `_transitions` 也證實 `packed` 同時可以直接跳 `done`，也可以先過 `shipped` 再到 `done`：**Shipped 是一個「可選」的中繼檢查點**，對應「貨真的離開倉庫、交給物流／貨運業者手上」這個動作本身；如果業務流程不需要另外追蹤「已交運但客戶還沒收到」這個時間差（例如同城/自取，出貨即完成），可以直接點 **COMPLETE** 跳過 Shipped
- 這跟 Day 3 學到的 Internal Shipment 判斷邏輯是同一個道理：**只有真的需要「這個中繼狀態」代表的商業意義時，狀態機才會走過那一階；不需要的話，Tryton 允許直接跳過**（Internal Shipment 沒有 `transit_location` 時跳過 Packed／Shipped，這裡是同一家書店、同倉庫出貨，不需要 Shipped 這個「已交給貨運公司」的中繼點，直接 COMPLETE 即可）
- 這次選擇：直接點 **COMPLETE**，不特地點 Ship

**實測確認（2026-07-09）：點 COMPLETE 之後，貨真的到客戶手上了**

- 📸 `day4-user-10-shipment-done-state.png`（State=Done，Effective Date 自動填 07/09/2026，footer 按鈕全部消失——離開 Draft 之後的唯讀畫面，不是卡關）

**執行結果（psql 複查，2026-07-09）**

| 表 | 欄位 | 值 |
|---|---|---|
| `stock_shipment_out` | id / state / effective_date | 1 ／ `done` ／ 2026-07-09 |
| `stock_move` ×2 | id / state / effective_date | 7／6 皆為 `done` ／ 2026-07-09 |
| `sale_sale` | state / invoice_state / shipment_state | `processing` ／ `pending` ／ **`sent`** |

**淨庫存最終結果（psql 複查，2026-07-09）**

| Location | 出貨前 | 出貨後 |
|---|---|---|
| Storage Zone | 4 | 4（不變，Pick 那步已經扣過了） |
| Output Zone | 5 | **3**（2 本交運出去） |
| Customer | 0 | **2**（客戶手上的書，第一次在這門課出現「Customer」這個 Location 有真實庫存） |

**概念筆記：`sale_sale.state` 還是 `processing`，不是 `done`——為什麼？**

- 現象：Shipment 已經 `done`，書也確實到了 Customer 手上，但 Sale 本身狀態沒有跟著變成 `done`，`shipment_state` 倒是變成了 `sent`（已出貨）
- 查了 `_process_state()`（`process()` 最後一步會呼叫）：Sale 要整張單變成 `done`，必須**同時**滿足 `shipment_state` 跟 `invoice_state` 都到達各自的終態——`shipment_state` 現在確實到了 `sent`（出貨這條線已經走完），但 `invoice_state` 還停在 `pending`（發票只是產生了草稿，還沒過帳／收款）
- **業務意義**：一張銷售單真正「完成」，兩條腿（出貨、收款）都要走到底，缺一不可——這正好銜接到 **Day 5（發票與收付款）**：現在這張 Sale 差的最後一塊，就是把那筆已經產生的草稿發票（`account_invoice` id=2）走完 Post／收款流程

至此 Day 4（銷售到出庫）主線走完：Step 1 把商品設成可銷售、Step 2 建銷售單並走完 Draft→Quotation→Confirmed→Processing（含「Confirm 自動 Process」的機制驗證）、Step 3 走完 Customer Shipment 的完整七階狀態機（Assign→Pick→Pack→Done），親眼看到庫存從 Storage Zone 一路流動到 Customer 手上。剩下的 Invoice／Payment 留給 Day 5 收尾。

## Day 5｜發票與收付款

- 狀態：🔄 進行中（2026-07-13 開始）
- 前置狀態確認（psql）：
  - `account_invoice` 已有兩筆草稿發票，都是 Day 2／Day 4 的 Process 自動產生的，還沒有人工建立過任何發票：
    | id | type | party | 狀態 | 對應單據 |
    |----|------|-------|------|---------|
    | 1 | in（供應商發票）| party=2 | draft | 對應 Day 2 的 Purchase Order |
    | 2 | out（客戶發票）| party=3 | draft | 對應 Day 4 的 Sale Order |
  - 兩筆發票明細（`account_invoice_line`）都指向 product id=2（`bk001`），金額分別是採購 10×200、銷售 2×680；使用的科目分別是：
    - 採購發票 → account id=112「Cost Of Sales」（type=Cost of Sales，損益表科目）
    - 銷售發票 → account id=99「Goods」（type=Revenue，損益表科目）
    - 這兩個科目都是從 `product_category_account`（書籍分類）的 `account_expense`/`account_revenue` 自動帶出來的——這正是 `business_flow.md` 提到的 `account_product` 模組的作用：產品自動對應收入/費用科目，不用手動指定

### 概念釐清：Supplier Invoice 上 Taxes 欄位顯示 `(0)`，這樣正常嗎？

📸 `day5-user-01-supplier-invoice-draft-tax-zero.png`（打開 Day 2 那張採購發票，Lines 裡 Taxes 欄位顯示 `(0)`）

- **`(0)` 是「這行沒有勾任何 Tax」，不是「稅算出來是 0」**——這兩件事很容易搞混
- psql 查證：這個環境的 `account_tax` 表**完全是空的（0 筆）**，`product_category-supplier-account_tax`／`product_category-customer-account_tax` 這兩張掛稅碼規則的表也都是 0 筆——不是漏設定，是從頭到尾就沒有建立過任何稅碼
- 原因：Day 1 建 Chart of Accounts 時用的是「Universal」範本，這個範本本身不含任何預設稅碼（不像有些國別範本會內建加值稅／營業稅科目）；`business_flow.md` 的規劃也只提到「用最簡單的固定價」，沒有涵蓋稅務設定
- **結論：這裡的 tax=0 是符合這門課範圍的預期狀態，不是卡關，可以直接 Post**
- 真實導入案子的話，會需要另外建 `account.tax` 稅碼（例如台灣的營業稅 5%），再掛到 product category 或 party 的 fiscal position 上，Tryton 才會在開發票時自動算稅——但這是這門課刻意先跳過的部分

### 概念釐清：按 POST 是什麼商業意義？

查了 `trytond/modules/account_invoice/invoice.py` 的 `post()`／`_post()`／`get_move()`，Post 不只是「排版鎖定」，它做了幾件會計上關鍵的事：

1. **指定發票編號**（`set_number`）——跟 Day 2／Day 4 的 Sale／Purchase Number 是同一套「離開草稿才給編號」的模式
2. **產生真正的傳票（`account.move`）**：`get_move()` 把發票明細（例如採購發票的 10 本 × 200 = 2,000，科目 Cost Of Sales）轉成一組借貸分錄，另一邊用 Payment Term 算出對應的**應付／應收帳款**分錄配平（`_get_move_line`：Supplier 發票是貸方 Payable，Customer 發票是借方 Receivable）
3. **把這張傳票過帳（`Move.post(...)`）**——這一步之後，這筆交易才會真正出現在總分類帳／試算表上，才會反映到損益表（費用／收入增加）跟資產負債表（應付／應收帳款增加）

白話講：Draft 發票只是「草稿」，對會計帳完全沒有影響；**Post 才是這筆交易真正「入帳」的那一刻**——Day 5 最後一步要看損益表／資產負債表，能不能看到這筆交易，就是靠這一步。

**實測踩到的錯誤：Post 供應商發票時擋下「"Invoice Date" is required」**

📸 `day5-user-02-supplier-invoice-post-error-invoice-date.png`（Pre-validation 錯誤：`"Invoice Date" is required. Type: Supplier Type: !Supplier`）

- 查了原始碼，`post`（跟 `validate_invoice`）按鈕的 `pre_validate` 規則寫死了：`['OR', ('invoice_date', '!=', None), ('type', '!=', 'in')]`
- 白話翻譯：**只有 Supplier（採購）發票才強制要求 Invoice Date 一定要填**，Customer 發票沒有這條硬性限制。錯誤訊息裡的 "Type: Supplier Type: !Supplier" 其實就是介面把這條 domain 規則直接轉成文字顯示，讀起來很怪，但意思就是「當 type=Supplier 時必填」
- **商業意義**：因為這是「別人（供應商）開給你的發票」，系統不能自作主張填「今天」當發票日期，一定要人工填入供應商發票上實際印的開票日期——這個日期會拿去算 Payment Term 到期日、決定入帳的會計期間（Period），是真正的稽核依據，不能用系統操作日期取代
- **對照**：Customer 發票不強制填 Invoice Date，是因為那張發票的開立日期就是「本公司自己決定的」，系統可以合理地在 Post 時自動代入今天
- 解法：回到發票表單填上 Invoice Date（實務上填供應商發票的實際開票日；這門課填今天 2026-07-13），存檔後再按一次 POST

**實測確認（2026-07-13）：填完 Invoice Date 後 Post 成功**

📸 `day5-user-03-supplier-invoice-posted.png`（State 已變成 Posted，Untaxed/Total 都是 2,000.00）

psql 覆查（`account_invoice` id=1）：

| 欄位 | 結果 |
|------|------|
| number | `2026-SI-00001` |
| state | `posted` |
| invoice_date | `2026-07-13` |
| move | `3`（已產生對應傳票） |

`account_move` id=3 的分錄（`account_move_line`），跟前面「概念釐清」講的 `get_move()` 機制完全對上——一借一貸，剛好配平：

| account | 科目名稱 | debit | credit |
|---------|---------|-------|--------|
| 112 | Cost Of Sales | 2,000.00 | 0 |
| 67 | Trade Payables | 0 | 2,000.00 |

這證實了 Post 的真正效果：借方費用（Cost Of Sales）2,000，貸方應付帳款（Trade Payables）2,000，這筆交易現在已經「入帳」，會出現在損益表（費用面）跟資產負債表（負債面）上——接下來的「登錄付款」就是要把這筆 Trade Payables 沖銷掉。

### 概念釐清：Post 完之後，Payment 分頁沒有 PAY 按鈕，卡住了

📸 `day5-user-04-supplier-invoice-payment-tab-no-pay-button.png`（Payment 分頁，`Lines to Pay` 顯示到期日 2026-08-12、應付 2,000，但 `Payment Lines` 是空的，畫面下方只有 CANCEL／PROCESS，沒有 PAY）

- **這個 Payment 分頁不是登錄付款的地方**——它顯示的是這張發票依 Payment Term 算出來的到期日跟金額（`Lines to Pay`），下面的 `Payment Lines` 目前是空的（0 筆），代表還沒有任何實際付款紀錄。真正登錄付款要靠表單下方的 **PAY** 按鈕，但這個按鈕整個沒有出現
- 查了 `account_invoice/invoice.py` 的 `get_has_payment_method()`：Pay 按鈕的顯示條件是 `has_payment_method=True`，而這個值是去查 **`account.invoice.payment.method`** 這張表**至少要有一筆**符合條件的紀錄（`debit_account`／`credit_account` 都不等於這張發票的 account）
- psql 查證：`account_invoice_payment_method` 這個環境目前是 **0 筆**——從頭到尾沒有人設定過任何付款方式，所以不是 aaa 權限不夠，是這個環境本來就缺這塊設定，跟 Day 2 缺 Shipment、Day 3 缺庫存是同一種「前面沒人做過這件事」的模式
- 查了 `ir_model_access`，`account.invoice.payment.method` 只有 **Accounting Administration** 群組能建立/編輯，aaa 現有的 **Accounting** 群組不夠，這一步要請 admin 執行
- **結論**：已經請 admin 去 Financial ‣ Configuration ‣ Invoices ‣ Payment Methods 建一筆（Journal=Cash，Debit/Credit Account=Cash and Cash Equivalents），詳見 [`admin-guide.md`](admin-guide.md) 「Day 5 前置（續）：建立 Invoice Payment Method」
