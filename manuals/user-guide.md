...
- **文件更正**：`tryton_business_flow.md`／`user-guide.md` 原本把 Day 2 流程簡化寫成「建單 → 確認」，漏講了中間的 Quotation 這一階，這裡補正確
...
- **完整狀態機更正**：`Draft → Quotation → Confirmed → Processing → Done`，其中 **Quote 給號**、**Process 才真正產生 Supplier Shipment**，跟原本 `tryton_business_flow.md` 寫的「建單 → 確認 → 系統產生 Supplier Shipment」又更精確一階——要點完 **Process** 才會看到 Shipment 出現
...
- 狀態：🔄 進行中——單據建好後在 Assign 卡關，原本預期 Day 2 做完就能重試成功，**但 Day 2 完成後發現預期落空**（Input Zone 已被自動上架流程清空），見下方「更新」區塊的修正建議，之後重開 Day 3 時再處理，先不用急著重試
...
**更新（2026-07-08，Day 2 做完後回頭看，原本的預期落空了）**：Day 2 的 Supplier Shipment 走完 Receive → Complete 後，psql 複查發現 Input Zone 的淨庫存又變回 **0**——因為 `receive()` 會自動產生並做完「Input Zone → Storage Zone」的上架 Move，貨已經直接被系統搬進 Storage Zone，**不會停留在 Input Zone 等人工搬運**。現況：

| Location | bk001 淨庫存（psql 複查 done moves 推算） |
|---|---|
| Input Zone | 0 |
| Storage Zone | 10 |

- 這張還停在 Waiting 狀態的 Internal Shipment（id=1，From=Input Zone、To=Storage Zone、Qty=5）**現在重新點 Assign，預期依然會失敗**——不是因為還沒做 Day 2，而是因為它想搬的來源地已經被 Day 2 的自動上架流程清空了
- **這其實是個蠻好的體會**：這張單原本想練習「驗收後上架」這個動作，但 Tryton 的 Supplier Shipment 流程已經把這個動作自動做掉了，不需要再手動用 Internal Shipment 模擬一次——真實情境裡，Internal Shipment 通常是用在**已經上架的庫存之間**再搬動（例如分店調貨、換儲位），不是「進貨後從 Input 搬進 Storage」這種已經被自動化的步驟
- **恢復 Day 3 時的建議做法（先不用執行，等真的要做 Day 3 再處理）**：
  1. 把這張舊的 Internal Shipment 點 **Cancel** 作廢，不用留著等一個不會發生的 Assign 成功
  2. Day 3 Step 2 重新示範時，改用**兩個都在 Storage Zone 底下、但邏輯上分開的地點**（例如如果之後有第二個門市/倉庫，或利用 Storage Zone 的子地點）來練習 Internal Shipment，才會是「已上架庫存之間搬動」這個更貼近真實的情境；或者單純示範「From=Storage Zone、To=Storage Zone 的某個子位置」也可以，只要來源地真的有庫存（現在 Storage Zone 有 10 本 `bk001`，可以拿來練習）
  3. Step 3（盤點）不受影響，Storage Zone 現在有真實庫存（10 本），可以正常練習「盤點抓差異」
